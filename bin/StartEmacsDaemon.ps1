...
$env:EMACS_LOG_DIR="$env:SERVER_HOME\logs"
...
Set-Content -Path $env:EMACS_LOG_DIR\StartupPSLog.txt -Value (([datetime]::Now.ToString("HH:mm:ss.fff yyyy-MM-dd")) + (" Script started"))
...
if(Test-Path $env:SERVER_HOME\server\){
	...
	Add-Content -Path $env:EMACS_LOG_DIR\StartupPSLog.txt -Value (([datetime]::Now.ToString("HH:mm:ss.fff yyyy-MM-dd")) + (" Script removed old server"))
} else {
    # new server, go
	Add-Content -Path $env:EMACS_LOG_DIR\StartupPSLog.txt -Value  (([datetime]::Now.ToString("HH:mm:ss.fff yyyy-MM-dd")) + (" Old server not found "))
}

Add-Content -Path $env:EMACS_LOG_DIR\StartupPSLog.txt -Value (([datetime]::Now.ToString("HH:mm:ss.fff yyyy-MM-dd")) + (" Starting server"))
#
# Do it
#
runemacs --daemon
...
Add-Content -Path $env:EMACS_LOG_DIR\StartupPSLog.txt -Value (([datetime]::Now.ToString("HH:mm:ss.fff yyyy-MM-dd")) + (" New emacs daemon has Process Id " + ((get-process "emacs").Id)))
Add-Content -Path $env:EMACS_LOG_DIR\StartupPSLog.txt -Value (([datetime]::Now.ToString("HH:mm:ss.fff yyyy-MM-dd")) + (" Daemon started at " + ((get-process "emacs").StartTime)))
